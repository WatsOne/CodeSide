...
class MyStrategy {
    ...
    fun getAction(unit: model.Unit, game: Game, debug: Debug): UnitAction {
        ...
        action.velocity = -10.0;//targetPos.x - unit.position.x
        action.jump = jump
        ...
        debug.draw(CustomData.Log("SPEED>>>: ${action.velocity}"))
        debug.draw(CustomData.Log("CERRENT>>>: ${unit.position.x}"))
        debug.draw(CustomData.Log("CERRENT_Y>>>: ${unit.position.y}"))
        return action
    }
